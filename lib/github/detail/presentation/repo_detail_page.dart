  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            state.maybeMap(
                ...
                loadSuccess: (s) {
                  return IconButton(
                    onPressed: !s.repoDetail.isFresh
                        ? null
                        : () {
                            ref
                                .read(repoDetailNotifierProvider.notifier)
                                .switchStarredStatus(s.repoDetail.entity!);
                          },
                    icon: Icon(
                      !s.repoDetail.isFresh
                          ? MdiIcons.starRemoveOutline
                          : s.repoDetail.entity?.starred == true
                              ? Icons.star
                              : Icons.star_outline,
                    ),
                  );
        ...
        body: state.map(
          initial: (_) => Container(),
          loadInProggress: (_) => const Center(
            child: CircularProgressIndicator(),
          ),
          loadSuccess: (s) {
            if (s.repoDetail.entity == null) {
              return const NoResultsDisplay(
                  message: 'Wow wow nothing to show about this repo right now');
            } else {
              return WebView(
                javascriptMode: JavascriptMode.unrestricted,
                navigationDelegate: (navReg){
                  if (navReg.url.startsWith('data:')){
                    return NavigationDecision.navigate;
                  } else {
                    url_launcher.launch(navReg.url);
